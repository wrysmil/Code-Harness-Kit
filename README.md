# Code-Harness-Kit · 代码马具套件

![Code-Harness-Kit Banner — Production AI Agent Harness for Claude Code & Cursor](image/banner.png)

<p align="left">
  <a href="./LICENSE"><img src="https://img.shields.io/badge/License-MIT-yellow.svg" alt="License: MIT"></a>
  <a href="./.agents/agents/"><img src="https://img.shields.io/badge/Worker_Agents-11-blue.svg" alt="Worker Agents"></a>
  <a href="./.agents/skills/"><img src="https://img.shields.io/badge/Skills-29-blue.svg" alt="Skills"></a>
  ...
  <a href="#快速接入"><img src="https://img.shields.io/badge/安装-5分钟-success.svg" alt="Install in 5 min"></a>
</p>

> **Production-grade AI Agent Harness for Claude Code & Cursor**
> 把项目规则、路由判定、阶段门禁、过程产物、集体测试与三层并行审查、双层适配与工具链打包成一套脚手架 —— 让 LLM 像一支有纪律的工程团队一样交付软件，而不是"单 Agent 一把梭哈"。

> **核心价值**：解决 LLM 代码生成"可靠性、一致性、长期可维护性"三大顽疾。Harness 像马具 —— 用缰绳与鞍具把马力引到正确方向，而不是限制能力本身。
>
> **核心理念**：**Agent Team · 阶段门禁 · 集体测试 · 集体审查 · 文档驱动**——五件套协同。

---

## TL;DR

- 🎯 **给 AI 上缰绳**：把"AI 单兵写代码"升级为"工程团队协作"，自带阶段门禁与尾盘强制审查
- 🤖 **跨平台适配**：同一套规则同时跑在 Claude Code / Cursor / Trae，业务项目零侵入接入
- 🌳 **Git Worktree 隔离**：每个 WU 独立 worktree，文件所有权清晰，并行不打架
- 🧪 **集体测试 + 三层审查**：批次收尾强制 code review / OWASP / CWV 并行扇出，不靠"应该通过"
- 📜 **文档驱动**：所有过程产物落盘 `.ai-runtime-artifacts/`，跨会话可接续

**30 秒判断要不要用**：你在用 Cursor / Claude Code 写代码，但担心 LLM 跑偏、PR 质量不稳 → 装上它。如果你只是写单文件 typo，它太重 —— 用 Claude Code 原生即可。
...
- [这是给谁的](#这是给谁的)
- [它解决什么问题](#它解决什么问题)
- [为什么是 Harness（对比 CrewAI / AutoGen / 裸用 Cursor）](#为什么是-harness对比-crewai--autogen--裸用-cursor)
- [五大设计理念](#五大设计理念)
- [架构设计](#架构设计)
...
- 一次性单文件 typo / 改常量（用 Tier 0 直接改更快）
- 你只想跑单 LLM 不需要编排（直接用 Claude Code / Cursor 原生）
- 项目还没用任何 AI 编程工具（先从 Claude Code 开始）
...
## 为什么是 Harness（对比 CrewAI / AutoGen / 裸用 Cursor）

| 维度                                                     | **Code-Harness-Kit** | CrewAI / AutoGen / MetaGPT | Cursor / Claude Code（裸用） |
| -------------------------------------------------------- | :------------------------: | :------------------------: | :--------------------------: |
| **阶段门禁** (spec / plan 写入后必须暂停等确认)    |          ✅ 内置          |       ⚠️ 需自己写       |            ❌ 无            |
| **三层并行审查** (代码 / 安全 / 性能 三人同时审)   |          ✅ 强制          |       ⚠️ 仅代码层       |            ❌ 无            |
| **Git Worktree 隔离** (每个任务独立分支)           |          ✅ 强制          |       ⚠️ 需自己配       |        ⚠️ 平台原生        |
| **强制产物落盘** (`.ai-runtime-artifacts/`)      |     ✅ FM + Next 契约     |       ⚠️ 散落各处       |      ⚠️ 平台私有目录      |
| **平台无关** (同一套规则跑 Claude / Cursor / Trae) |        ✅ 单一规则        |         ✅ 多 LLM         |        ❌ 绑死单平台        |
| **防幻觉机制** (没跑测试就说完成 = 无效)           |          ✅ 铁律          |       ⚠️ 文档约定       |            ❌ 无            |
| **目标场景**                                       |       长周期软件交付       |   单次研究 / 一次性任务   |         探索性写代码         |

**一句话区别**：CrewAI / AutoGen 解决"LLM 怎么协同"，Harness 解决"LLM 在真实软件工程里怎么不翻车"。
...
### Q1. Code-Harness-Kit 和 CrewAI / AutoGen / MetaGPT 有什么区别？

CrewAI / AutoGen 解决"多个 LLM 怎么协同跑任务"（研究型、一次性任务友好）；Code-Harness-Kit 解决"LLM 在真实软件工程长周期交付里怎么不翻车"（spec → plan → WU 派发 → 三层审查 → 产物落盘）。如果你要做一个 2048 游戏用 MetaGPT 更快；你要做一个生产级 SaaS 长期迭代，用 Harness。

### Q2. 支持哪些 AI 编程平台？

目前完整支持 **Claude Code** 与 **Cursor**；**Trae** 仅骨架（实验性）。Harness 的设计是平台无关 —— 加新平台只需在 `platform/<name>/` 下写一份 binding，无需动 `core/`。

### Q3. 接入需要多久？会污染业务代码吗？

5-10 分钟。Harness 把所有规则放在 `harness-kit/` 子目录 + 根 `AGENTS.md` 一个指针文件，**完全不污染** `src/`、`tests/` 等业务目录。升级 harness-kit 与提交业务代码**完全解耦**。

### Q4. 必须用 Git Worktree 吗？能在主分支直接写吗？

Tier 0 / Tier 1 单文件小改动可在主 checkout 直接做。Tier 2+ 编排调度**强制** worktree 隔离 —— 这是为了避免并行 WU 互相改坏文件。Worktree 在 `git worktree add` 时创建，批次完成后 `git worktree remove` 清理。

### Q5. 阶段门禁会不会让 AI 变慢、流程变重？

短期看：多了一次确认步骤。长期看：避免"未批先写"导致返工 —— 一行 spec 写错，写完 500 行代码再回滚的代价远大于多等 30 秒确认。这是**为一致性付的小成本**。

### Q6. Leader 不亲自写代码，AI 还能交付吗？

Leader 是技术主管 / 编排者，**不亲自写大规模业务代码**（Tier 1 简单实现除外）。所有编码走 Worker（W1.5: Coder 子代理），Leader 做路由判定 + 阶段门禁 + 整合 + 尾盘审查。这是模拟真实工程团队 —— Tech Lead 不写代码，但负责不让代码烂。

### Q7. 怎么贡献或反馈问题？

- 🐛 Bug / 改进：直接开 [GitHub Issues](../../issues)
- 💬 想法 / 讨论：详见 [致谢](#致谢) 章节列出的参考项目风格
- 🔧 提 PR 前：跑 `bash scripts/harness-check.sh` 通过 + 附产物到 `.ai-runtime-artifacts/`

### Q8. 商业使用有什么限制？

MIT License —— 商用、私有 fork、改名再发布都允许，唯一要求是保留版权声明。详见 [LICENSE](./LICENSE)。
...
| 优先级 | 项                                                                      | 状态   |
| ------ | ----------------------------------------------------------------------- | ------ |
| 🔴 P0  | Trae 适配器收尾（目前仅有骨架）                                         | 规划中 |
| 🔴 P0  | `oh-my-claudecode` 多 CLI Worker 调度（Codex / Gemini / Antigravity） | 规划中 |
| 🟠 P1  | 性能审计 Agent 增强（CWV 阈值、Bundle budget、INP 长任务）              | 进行中 |
| 🟠 P1  | 真实 Claude Code 会话截图 + 文档站                                      | 进行中 |
| 🟡 P2  | 远程协作（多 Leader 同步同一 worktree 池）                              | 评估中 |
| 🟡 P2  | VS Code 扩展（一键接入 harness-kit）                                    | 评估中 |

> 想投票 / 提需求 → [GitHub Issues](../../issues)
...
| [.agents/agents/security-auditor.md](.agents/agents/security-auditor.md)               | Security-Auditor prompt（OWASP + LLM 安全）       |
| [.agents/agents/perf-auditor.md](.agents/agents/perf-auditor.md)                       | Perf-Auditor prompt（CWV + N+1 + Bundle）         |
