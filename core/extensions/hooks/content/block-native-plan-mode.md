...
替代流程：写实施计划必须 Load `superpowers:writing-plans` skill 并 `Write` 到 `.ai-runtime-artifacts/plans/YYYY-MM-DD-<topic>-plan.md`（并行时另写同 stem `*-dispatch.md`）。详见 `harness-kit/core/routing.md` § 平台原生 plan 工具 与 `platform/claude/bindings.md`。
